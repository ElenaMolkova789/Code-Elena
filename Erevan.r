...
erevan = data.frame(id="Erevan",
                      latitude = c(40.196345),
                      longitude= c(44.520704))


stations = meteo_nearby_stations(lat_lon_df = erevan, station_data = stations,
                                        limit = 20, var = c("PRCP"),
...
# удалленности от парка, очевидно что первым элементом таблицы будет идентификатор метеостанции Сочинского Дендрария, его то мы и попытаемся получить
erevan_id = stations$Erevan$id[2]
#Для получения всех данных с метеостанции, зная ее идентификатор, используйте след. команду

one_station=meteo_tidy_ghcnd(
  "RSM00037099")
all_erevan_data = meteo_tidy_ghcnd(stationid = erevan_id)
all_erevan_data=rbind(one_station)

all_erevan_data = all_erevan_data %>% mutate(year = year(date)) %>% 
  filter(year > 2010 & year < 2019) %>% 
  mutate(tavg = tavg/10, prcp = prcp/10) %>% dplyr::select(-tmax,-tmin)

all_erevan_data$prcp[is.na(all_erevan_data$prcp)] = 0 
erevan_cum = all_erevan_data %>% mutate(month = month(date)) %>% 
  filter(month > 3 & month < 10) %>% 
  group_by(year) %>% 
  mutate(prcp_cum = cumsum(prcp))

erevan_cum %>% summarise(prcp_avg = max(prcp_cum), n = n())
...
park_sf2 <- read_sf(str_interp('ahtanak.geojson'))
...
dates = MODISTools::mt_dates(product = "MOD16A2", lat = 40.196345, lon = 44.520704)
...
# 
erevan_ET = erevan_ET %>% filter(value < 32700) %>% select(units,calendar_date,value) %>%
...
Prcp_cum = erevan_cum %>% filter(year == 2013) %>% mutate(doy = yday(date)) %>% 
